...
NotDeft is derived from Deft version 0.3, but differs in several notable ways:
  ...
  2. NotDeft chooses notes of interest by invoking a =notdeft-xapian-program=, which uses the Xapian library to implement free-text search across note files, with convenient query syntax. The search is performed across all =notdeft-directories=, and in a =notdeft-mode= buffer further narrowing down of the result set can then be done incrementally by typing in a search string for purposes of filtering (as in Deft).
     - That is, the file browser of NotDeft lists Xapian search results instead of listing directory contents.
  3. NotDeft supports the existence of multiple =notdeft-mode= buffers within a single Emacs instance, so that one can view and operate on multiple search result sets of notes simultaneously.
  4. NotDeft includes multiple functions and commands intended to be usable from outside =notdeft-mode=, leading to more complex modalities of use than with Deft's list view centric operation.

NotDeft's major =notdeft-mode= is not Deft---the complication of having two stages (query, then filter) to the process of looking for interesting files makes the mode less simple and intuitive than Deft's =deft-mode=, as does having multiple directories, modalities, and buffers.
...
NotDeft can be installed either manually, or as a package with a tool such as [[https://github.com/radian-software/straight.el][straight.el]]. Instructions for such installations and their pre-requisites are given in the "README.org" file, and this section provides additional detail. There is also an installation test program in the "init-example" directory of the source distribution, which also serves to demonstrate some of the different ways of setting up NotDeft.

** Installation as a Package with straight.el

The straight.el package manager is able to install NotDeft as a package directly from its source repository. With that manager it should be possible to install NotDeft with the command
...
The NotDeft source repository also includes optional packages, kept separate due to their additional dependencies (on [[https://github.com/abo-abo/swiper][Ivy]] and [[https://github.com/magit/transient][Transient]]). If desired, they can be also be installed with similar =straight-use-package= invocations:
#+begin_src emacs-lisp :results silent
  (straight-use-package
   `(notdeft-ivy
     :type git :host github :repo "hasu/notdeft"
...
While the above commands fetch, build, and set up NotDeft's Emacs Lisp code, they do not build and configure the C++-based Xapian backend; see [[*Building the Xapian Backend][Building the Xapian Backend]] and [[*Configuring the Xapian Backend][Configuring the Xapian Backend]].

** Installation as a Package with =use-package :straight=

When using =use-package= together with straight.el, one can also specify the =:straight= package recipe within the package use form:
#+BEGIN_SRC emacs-lisp :results silent
  (use-package notdeft
   :straight (notdeft
	      :type git :host github :repo "hasu/notdeft"
...
One can then also do package configuration with the same =use-package= declaration, for example by including a =:config= directive to [[#notdeft-xapian-make-program-when-uncurrent][automatically build the required C++ program as necessary]], as shown in "README.org".

** Building the Xapian Backend

To enable Xapian search queries in NotDeft it is necessary to build the =notdeft-xapian= C++ program in the “xapian” directory. On some systems simply going into that directory and typing
#+begin_src shell
  make
#+end_src
should do the trick, provided that the required tools and libraries have already been installed. Other systems may require more work not only on satisfying the dependencies, but also in finding the right C++ compiler incantation for building the program. (If it cannot be built, perhaps due to the computer being locked down and without a suitable compiler, then in that case NotDeft will not be usable.)
...
: M-x notdeft-xapian-make-program
which should display any build errors if the executable cannot be built.

*** Building the Backend Automatically
:PROPERTIES:
:CUSTOM_ID: notdeft-xapian-make-program-when-uncurrent
:END:

The =notdeft-xapian-make= Emacs Lisp feature also includes a mechanism for building the Xapian backend program whenever it is out of date with respect to its sources, or does not exist at all. This can be useful whenever one has used =git pull= to fetch a more recent “notdeft-xapian.cc”, as then one need not remember to also recompile the executable.

To use the feature to do =notdeft-xapian= builds on NotDeft startup as necessary one can do
#+BEGIN_SRC emacs-lisp :results silent
...
If you =package-install='ed NotDeft, and built the =notdeft-xapian= executable in that location, then the appropriate setting may be
#+BEGIN_SRC emacs-lisp :results silent
...
- The default is to order the results so that most recently edited files are listed first, but you may change this behavior by setting =notdeft-xapian-order-by= to =relevance=, in which case Xapian's ranking mechanism is used instead.
...
To open a =*NotDeft*= buffer directly with a particular search query, use the command =notdeft-mode-open-query= from any buffer.
...
** Enabling NotDeft Note Mode Conditionally Based on Major Mode

Perhaps a better approach is to enable the mode for certain major mode(s), but only if the file looks like a note file based on its name and directory. The =notdeft-config= feature implements a simple hook function for doing such conditional enabling =notdeft-note-mode=:
#+BEGIN_SRC emacs-lisp :results silent
...
A different solution is to try enabling =notdeft-note-mode= for every NotDeft /directory/ in terms of [[https://www.gnu.org/software/emacs/manual/html%255Fnode/emacs/Directory-Variables.html][per-directory local variables]]. For example, have your “.dir-locals.el” file state
...
#+END_SRC
after which the command =[f6] o= should invoke the =notdeft-mode-open-query= command in any mode that does not override the binding for F6 with something else.
...
#+BEGIN_SRC emacs-lisp :results silent
  (defun my-open-todo-notes ()
    (interactive)
    (notdeft-open-query :query "tag:todo"))
#+END_SRC
...
** Query Modifiers
   ...
   - =!time= :: Prefix a query with =!time= to have the results sorted by decreasing file modification time, even if the =notdeft-xapian-order-by= configuration option is not set to =time=, which is the default setting.
   - =!rank= :: Prefix a query with =!rank= to have the results sorted by decreasing relevance.
   - =!file= :: Prefix a query with =!file= to have results sorted by (non-directory) file name, alphabetically, in decreasing order.
...
If it seems hard to remember the various NotDeft commands, one may wish to have a command selection dialog, similar to the one in [[https://magit.vc/][Magit]]. For implementing such “helpful key bindings,” one can use Transient or [[https://github.com/abo-abo/hydra][Hydra]], for instance. As examples, the “extras” directory of NotDeft's source repository contains a predefined transient and hydra for NotDeft's mode-agnostic commands, provided by the =notdeft-global-transient= and =notdeft-global-hydra= features. For example, to bind =[f6]= to the hydra (instead of the =notdeft-global-map= keymap directly), one can use the configuration code
#+BEGIN_SRC emacs-lisp :results silent
...
The =notdeft-org= feature also defines a =notdeft-org-store-note-deft-link= command, which functions similarly to =org-store-link=, but stores a "deft:" link to the current note. In a NotDeft buffer, it stores a link to any selected note; and in a NotDeft note buffer, it stores a link to that buffer's note.
